...
public class GrayImg implements Processing {
	...
	int grayHeight, grayWidth;
	BufferedImage grayImage;
	
	public GrayImg() throws InterruptedException { //constructor fara parametri - apeleaza constructorul OriginalImg

		convertImg = new OriginalImg();
	}
	GrayImg(String y) throws InterruptedException { //constructor fara parametri - apeleaza constructorul OriginalImg
	...
	//se citeste numele fisierului prin intermediul metodei CitireFisier din clasa OriginalImg
		...
		//for(int i = 0; i < grayHeight; i++){
         ...
         //   }		
	///	}
		ImgStocata stored=new ImgStocata(grayWidth,grayHeight,grayImage);
		Producer p1=new Producer(stored, grayImage);
		Consumer c1=new Consumer(stored, grayImage);
		p1.start();
		c1.start();
		try {
            c1.join();
...
public BufferedImage ProcessToGreyscale(String y) throws InterruptedException //metoda ce va realiza conversia alb-negru
	
	{
		...
		//}
		ImgStocata stored=new ImgStocata(grayWidth,grayHeight,grayImage);
		Producer p2=new Producer(stored, grayImage);
		Consumer c2=new Consumer(stored, grayImage);
		p2.start();
		c2.start();
		try {
            c2.join();
